  ...
  try {
    ...
    if (fn === "top") {
      ...
      const list = Object.entries(j.data).filter(([k,v])=>k!=='date' && v && v.fluctate_rate_24H)
        .map(([k,v])=>({ symbol:k, chg:parseFloat(v.fluctate_rate_24H), vol:parseFloat(v.acc_trade_value_24H||0), closing_price:parseFloat(v.closing_price||0) }))
        .sort((a,b)=>b.chg-a.chg).slice(0, Number(n)||15);
