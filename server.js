...
app.get("/api/upbit", async (req,res)=>{
  try{
    ...
    if(fn==="markets"){
      ...
      const m=Number(req.query.minutes||1), market=req.query.market||"KRW-BTC", count=Number(req.query.count||200);
      url=`https://api.upbit.com/v1/candles/minutes/${m}?market=${encodeURIComponent(market)}&count=${count}`;
    }else{
